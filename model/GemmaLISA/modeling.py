        ...
        **kwargs,  # その他の不明な引数を受け取るためのキャッチオール
        ...
        if inputs_embeds is None and images is not None and input_ids is not None:
            ...
            if seg_token_mask.any() and self.lisa.model.sam_model is not None:
                ...
                # バッチサイズとして使用される次元の確認
                bs_sparse = sparse_embeddings.shape[0]  # sparse_embeddingsのバッチサイズ
                bs_dense = dense_embeddings.shape[0]    # dense_embeddingsのバッチサイズ
                bs_image = sam_image_features.shape[0]  # sam_image_featuresのバッチサイズ
                
                # 最小のバッチサイズを使用してすべてを調整
                min_bs = min(bs_sparse, bs_dense, bs_image)
                
                # バッチサイズを一致させる（必要な場合）
                if bs_sparse != min_bs:
                    sparse_embeddings = sparse_embeddings[:min_bs]
                    print(f"sparse_embeddingsのバッチサイズを調整: {bs_sparse} -> {min_bs}")
                
                if bs_dense != min_bs:
                    dense_embeddings = dense_embeddings[:min_bs]
                    print(f"dense_embeddingsのバッチサイズを調整: {bs_dense} -> {min_bs}")
                
                if bs_image != min_bs:
                    # 画像特徴量のバッチサイズがsparse_embeddingsより大きい場合、先頭部分のみ使用
                    if bs_image > min_bs:
                        sam_image_features = sam_image_features[:min_bs]
                        print(f"sam_image_featuresのバッチサイズを調整（切り詰め）: {bs_image} -> {min_bs}")
                    # 画像特徴量のバッチサイズがsparse_embeddingsより小さい場合、繰り返して拡張
                    elif bs_image < min_bs:
                        # 繰り返して拡張（bs_imageが1の場合はmin_bs回複製）
                        sam_image_features = sam_image_features.repeat(min_bs, 1, 1, 1)
                        print(f"sam_image_featuresのバッチサイズを調整（拡張）: {bs_image} -> {min_bs}")
                
                # バッチサイズ調整後のテンソル形状をログに出力（デバッグ用）
                print(f"調整後: sparse_embeddings形状={sparse_embeddings.shape}, dense_embeddings形状={dense_embeddings.shape}, sam_image_features形状={sam_image_features.shape}")
